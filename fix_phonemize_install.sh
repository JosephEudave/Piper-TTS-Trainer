#!/bin/bash
# Fix script for piper_phonemize installation issues
...
echo -e "${YELLOW}=========================================================${NC}"
echo -e "${YELLOW}Fixing piper_phonemize installation${NC}"
echo -e "${YELLOW}=========================================================${NC}"
...
echo -e "${YELLOW}Installing piper_phonemize...${NC}"
cd piper/src/piper_phonemize
...
cd ../../..
if python -c "import piper_phonemize; print('✅ piper_phonemize successfully imported!')" &>/dev/null; then
    ...
    TEST_RESULT=$(python -c "import piper_phonemize; print(piper_phonemize.phonemize('Hello world', 'en-us', True))" 2>&1)
    ...
    if [[ "$TEST_RESULT" == *"phonemes"* ]]; then
        ...
        echo -e "${YELLOW}Try running: export PHONEMIZE_ESPEAK_DATA=/usr/share/espeak-ng-data${NC}"
    fi
    
    echo -e "${GREEN}✅ Installation complete!${NC}"
else
    echo -e "${RED}❌ piper_phonemize installation failed${NC}"
    exit 1
fi 